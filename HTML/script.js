...
function existeUsuario(unUsuario){
    for (let i in listaImportadores){
        if (listaImportadores[i].usuarioEmpresa==unUsuario){
            return true
        }
    }
    for (let i in listaEmpresas){
        if (listaEmpresas[i].usuarioImportador==unUsuario){
            return true
        }
    }
    return false
    ...
    if(existeUsuario(nombre)==false){
        if(validarClave(contraseña)==true){
            if (contraseña==confirmarContraseña){
                let  importadorNuevo= new Importador(nombre.value,usuario,contraseña,foto);
